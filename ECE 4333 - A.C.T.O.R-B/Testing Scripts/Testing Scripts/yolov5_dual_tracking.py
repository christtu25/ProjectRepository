...
kit = ServoKit(channels=16, i2c_bus=1)  # Jetson Nano I2C bus 1
...
def track_person():
    ...
    # Initialize USB webcam (Logitech 1080p)
    usb_cap = cv2.VideoCapture(0, cv2.CAP_V4L2)  # Use V4L2 backend for USB cameras
    if not usb_cap.isOpened():
        print("Warning: Could not open USB webcam. Continuing with CSI camera only.")
        usb_cap = None
    else:
        # Set USB webcam resolution to 1920x1080 (adjust if needed)
    ...
    try:
        ...
        while True:
            ...
            # Read and display USB webcam frame (normal feed)
